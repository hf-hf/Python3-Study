...
LOG_FILENAME = 'p2_udp_port_health_check.log'
...
response_data = array.array('B',[0x29,0x29,0x21,0x00,0x05,0x8b,0x80,0x00,0x2f,0x0d])
...
def checkUdpPort(myconf):
    ...
    for udpServer in udpServerList:
        ...
        try:
            ...
            if(response_data == array.array('B', responseMsg)):
                print(responseMsg)
...
# TODO 重启端口
def proc_restart(ip, port, user, pwd, proc_port):
    restart_result = ''
    try:
        # 连接服务器
        ...
        if restart_result == '失败':
            mail_title = '责任人:张合 紧急度:高 ' + ip + ':' + proc_port + ' p2 udp端口监控报警需重启，自动重启失败，请立即确认！'
            #send_mail(mail_title, mail_title)
            return restart_result
        # 如果进程存在则先杀死进程
        if proc_num > 0:
            stdin, stdout, stderr = client.exec_command(
                "/root/1-runlatest-trunk-p2/2-stopContainer.sh %s" % (proc_port))
            outLines = stdout.readlines()
            errorLines = stderr.readlines()
            ...
            for std2 in errorLines:
                ...
                restart_result = '失败'
            if restart_result == '失败':
                mail_title = ip + ':' + proc_port + ' p2 udp端口健康检查程序出现异常，自动重启失败，请立即确认！'
                send_mail(mail_title, mail_title)
                return restart_result
        # 启动进程
        print("/root/1-runlatest-trunk-p2/1-startContainers.sh %s" % (proc_port))
        stdin, stdout, stderr = client.exec_command("/root/1-runlatest-trunk-p2/1-startContainers.sh %s" % (proc_port))
        outLines = stdout.readlines()
        errorLines = stderr.readlines()
        for std in outLines:
        ...
        send_mail(u'p2 udp端口健康检查程序出现异常，请确认！', u'请确认日志内容！')
